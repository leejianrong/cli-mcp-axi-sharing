...
Here's the reframe. The CLI-versus-MCP argument is really an argument about the protocol — about the plumbing. AXI, the Agent eXperience Interface, says the protocol isn't where the real lever sits. The lever is design discipline. And in practice AXI *is* a CLI — but one built with the agent as the primary user, not the human.
...
Principle one, token-efficient output. Before, we serialise the runs to pretty-printed JSON. After, one call — `renderOutput` — which encodes to TOON under the hood, and that alone is about forty percent smaller.
...
Here's the app. It's a tiny, fully offline CI/CD service — pipeline runs, the kind of thing an agent checks all day long. Did the build pass? Which runs are failing? What broke? There are eight seeded runs in there: three failed, two running, three passed.
...
Same data, same task. This is the four principles doing their job. That top line is a pre-computed summary, so the agent never burns a turn counting — that's principle four. Four fields instead of ten — principle two. The logs are truncated with a size hint — principle three — and here's the escape hatch, `--full`, which expands them when you actually need them. And the whole thing is encoded in TOON, about forty percent smaller than the JSON — principle one. [Point back at slide 8b.] That's the diff from a moment ago, running.
...
This is a recording, not a live agent. I want to be clear about that. But it *is* a genuine agent — Claude — doing the real task on the same app we just ran, three times, once through each interface. It's recorded for one reason: the numbers stay stable, and there's no network or API risk on stage. Same model, same task, same app — the only thing that changes is the interface, which is exactly the variable we care about.
...
These are author-published numbers, same caveat as before. But you just watched the mechanism live, and you watched it compound in a real run — and that's the reason to trust the direction, even if you'd want to reproduce the exact figures yourself.
