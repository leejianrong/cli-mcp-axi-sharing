...
| **Measured task** | Two levels: the **live tokenizer diff** uses the single call "list the failing runs" (per-call payload). The **recorded agent run** uses a **multi-step** task — "for each failing run, which job failed and is it flaky/infra or a real regression?" — where all three interfaces take the same fair **list + get-per-run** path; MCP re-reads its 21 schemas on every drill-in turn (schema tax compounds) while AXI's leaner payloads keep the whole task cheap. (`ci failures`, which answers the task in one call, is showcased separately as the AXI affordance payoff — not the measured path.) |
